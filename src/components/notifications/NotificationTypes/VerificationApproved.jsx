...
function VerificationApproved({ notification, onMarkAsRead, onDelete }) {
    const [deleting, setDeleting] = useState(false);

    const handleDelete = async (e) => {
        e.stopPropagation(); // Evitar que se active otros eventos
        setDeleting(true);
        await onDelete(notification.id);
    };

    return (
        <div className={`p-4 border-l-4 border-l-green-500 bg-white rounded-r-lg hover:bg-gray-50 transition duration-200 ${
