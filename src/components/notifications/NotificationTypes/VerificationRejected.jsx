...
function VerificationRejected({ notification, onMarkAsRead, onDelete }) {
    const [deleting, setDeleting] = useState(false);
    const actionRequired = notification.isActionable && notification.actionData?.actionRequired === 'resubmit_verification';
                    ...
                    {actionRequired && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 mt-2">
                            <p className="text-amber-700 text-xs font-medium">
                                ⚡ Puedes volver a enviar tu documentación
                            </p>
                        </div>
                    )}
